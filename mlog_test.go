...
var url = "your_slack_url"
var channel = "your_slack_channel_id"
...
func TestFileLogger(t *testing.T) {
	logger := New()
	fw := NewFileWriter("logs1/", "mlog_test", url, channel)
...
func TestAddWriter(t *testing.T) {
	fw := NewFileWriter("logs2/", "mlog_test", url, channel)
	err := AddWriter(fw)
	assert.Nil(t, err)
...
func BenchmarkFileLogger(b *testing.B) {
	logger := New()
	fw := NewFileWriter("benchlogs/", "mlog_test", url, channel)
	logger.AddWriter(fw)
	InitLogger(logger)
